...
const minProfit = minIndex >= 0 ? parseFloat(argv[minIndex + 1]) : 0.25;
...
function validatePrivateKeyFormat(k) {
...
async function createFlashbotsProvider() {
  ...
  if (!validatePrivateKeyFormat(authKey)) throw new Error('AUTH_PRIVATE_KEY appears invalid (expected 0x... 64 hex chars)');
  const authSigner = new ethers.Wallet(authKey);
  let FlashbotsBundleProvider;
  try {
    FlashbotsBundleProvider = (await import('@flashbots/ethers-provider-bundle')).FlashbotsBundleProvider;
  } catch (e) {
    throw new Error('@flashbots/ethers-provider-bundle is not installed. Install it or run without --flashbots');
  }
...
async function huntAndKill() {
  ...
  // Default borrow amount: use env AMOUNT if provided; otherwise use larger size for WETH for better hit-rate
  let defaultAmount = '1';
  try {
    const WETH_ADDR = COMMON.WETH.toLowerCase();
    if ((tokenA || '').toLowerCase() === WETH_ADDR) defaultAmount = (process.env.AMOUNT || '500').toString();
    else defaultAmount = (process.env.AMOUNT || '1').toString();
  } catch (e) { defaultAmount = (process.env.AMOUNT || '1').toString(); }
  const amountBorrowed = ethers.parseUnits(defaultAmount, decA);
  ...
  if (useFlashbots) {
    ...
    if (!validatePrivateKeyFormat(operatorPk)) {
      console.error('OPERATOR_PRIVATE_KEY appears invalid (expected 0x... 64 hex chars)');
      process.exit(1);
    }
  }
  const operatorWallet = (useFlashbots && operatorPk) ? new ethers.Wallet(operatorPk, provider) : null;
  ...
  while (running) {
    ...
    if (profitable && profitable.length > 0) {
      ...
      const calldata = flashIface.encodeFunctionData('flashArbitrage', [tokenA, amountBorrowed.toString(), [tokenA, tokenB], [tokenB, tokenA], '0']);
      ...
      if (dryRun) {
        ...
        console.log('DRY-RUN: printing opportunity and continuing scan.');
      }
